...
const Chat = () => {
  ...
  const chatterName = connections
    .filter((con) => con._id == targetUserId)
    .map((con) => con?.firstName + " " + con?.lastName);

  return (
    <div className="w-3/4 mx-auto h-[70vh] bg-slate-950 m-2 rounded flex flex-col">
      <h1 className="font-bold text-xl border-b-2 border-b-gray-400 p-2 text-white">
        {chatterName.join("")}
      </h1>
        ...
        {messages &&
          messages.map((msg, id) => (
            ...
            >
              ...
              <div className="chat-bubble text-white bg-black">{msg?.text}</div>
